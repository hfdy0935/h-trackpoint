...
function eventStore() {
  // 查询数据
  const query = ref<ReqEventList>({
    type: EventTypeEnum.DEFAULT,
    status: [StatusEnum.NORMAL, StatusEnum.DISABLED],
  ...
  const refresh = async () => {
    try {
      ...
      const resp = await reqEventList({
        ...
        projectIdList: query.value.projectIdList?.map(
          (el) => projectOptions.value.find((el1) => el1.name === el)!.id,
        ),
      })
      if (resp.code === 200) {
