...
import { useEventStore, useProjectStore } from '@/store';
...
const { query, filterEventField } = storeToRefs(useEventStore())
const start = (filterEventField.value === 'create_time' ? query.value.createTimePeriod : query.value.updateTimePeriod)?.start
const end = (filterEventField.value === 'create_time' ? query.value.createTimePeriod : query.value.updateTimePeriod)?.end
